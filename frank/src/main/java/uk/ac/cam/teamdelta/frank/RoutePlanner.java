...
public class RoutePlanner implements RouteFinder {
    ...
    private static Location moveByDist(Location loc, Direction bearing, double distance) {
        ...
        double lon2 = lon1 + Math.atan2(
                Math.sin(Math.toRadians(bearing.getDegrees())) * Math.sin(distance / earthRadius) * Math.cos(lat1),
    ...
    public JunctionInfo getNextPosition(Location current_position, Direction current_direction, double angleTolerance) {
        ...
        for (int i = 0; i < osmWays.size(); i++) {
            for (int j = 0; j + 1 < osmWays.get(i).nodeCount(); j++) {
                double curDist =
                        distFromSegment(current_position, osmWays.get(i).getNode(j).getLocation(), osmWays.get(i)
                                                                                                          .getNode(
                                                                                                                  j + 1)
                                                                                                          .getLocation());
                double curBearing =
                ...
                if (curDist < minDist && (
                        Math.abs(current_direction.getDegrees() - curBearing) < angleTolerance ||
                                ...
                                Math.abs((current_direction.getDegrees() - curBearing + 180.0) % 360) <
                                        angleTolerance ||
                                Math.abs((current_direction.getDegrees() - curBearing + 180.0) % 360) >
                                        (360 - angleTolerance)
                )) {
        ...
        int segOrder = 0;
        double segDist = 0;
        double segDir = bearing(osmWays.get(minDistWay).getNode(minDistNode).getLocation(),
                osmWays.get(minDistWay).getNode(minDistNode + 1).getLocation());
        ...
        while (moveDist > 0.1) {
            if (segOrder == 1) {
                if (curNode + 1 < osmWays.get(curWay).nodeCount()) {
                    segDist = dist(osmWays.get(curWay).getNode(curNode).getLocation(), osmWays.get(curWay)
                                                                                              .getNode(curNode + 1)
                                                                                              .getLocation());
                    segDir = bearing(osmWays.get(curWay).getNode(curNode).getLocation(), osmWays.get(curWay)
                                                                                                .getNode(curNode + 1)
                                                                                                .getLocation());
